  ...
  run(node) {
    ...
    for (const [source, dups] of dupsMap.entries()) {
      ...
      console.log(
        dups.map(
          dup =>
            `(${dup.loc.start.line}, ${dup.loc.start.column}) -> (${dup.loc.end
              .line}, ${dup.loc.end.column})`
        )
      );
