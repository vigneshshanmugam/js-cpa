...
program
  ...
  .option("-R, --report <type>", "Generate HTML report")
  .parse(process.argv);
...
if (program.filelist) {
  ...
  rl.on("close", () => {
    clearBuffer();
    try {
      const duplicates = print(cpa);
      if (program.report === "html") {
        generateHTMLReport(duplicates);
      }
    } catch (err) {
      errors.push({ err });
    }
...
function print(cpa) {
  ...
  // For the time being returning the values since
  // CPA function is doing something to the original object
  return duplicates;
}

function generateHTMLReport(duplicates) {
  const report = new HTMLReporter(duplicates, {});
  report.renderFile();
}
